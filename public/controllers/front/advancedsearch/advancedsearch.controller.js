var app = angular.module('app',['ngSanitize','ngRoute','ngAnimate','perfect_scrollbar','ui.select','ngCookies'], function($interpolateProvider) {
...
app.directive('searchFragmentsAdvanced', function() {
    return {
        templateUrl: '/templates/searchFragmentsAdvanced.html',
        link: function(scope, element, attributes, searchFactory){

            //console.log(attributes);
            scope.fragments = JSON.parse(attributes.searchFragmentsAdvanced);
...
app.controller('SearchNoAuthController',['$scope', '$http', '$log', '$q', '$location','AppService', '$window', '$filter', '$timeout', 'orderByFilter','$route','$routeParams', function($scope, $http, $log, $q, $location,AppService, $window, $filter, $timeout, orderBy,$route,$routeParams) {
    ...
    $scope.initOnLoad = function(){
        ...
        if($routeParams.frase!='' && $routeParams.stype) {
            ...
            //if (frasearray[1] && frasearray[1] != '') {
                ...
                if ($routeParams.stype == 'regex') {

                    $scope.data.search.frase = $routeParams.frase;
                ...
                } else if ($routeParams.stype == 'perfect') {
    ...
    $scope.changeSearchMode = function(mode){
        ...
        if(mode=='advanced'){
            $scope.changeSearchModeToAdvanced();
        }
    ...
    $scope.changeSearchModeToAdvanced = function(mode){
        $scope.data.search.searchmode = mode;
    }
    ...
    $scope.onSubmit = function(){
        ...
        if($scope.data.search.frase.length>0) {
            ...
            if($scope.is_perfect){
                $location.path('/').search('frase='+$scope.data.search.frase+'&stype=perfect');
            }else{
                $location.path('/').search('frase='+$scope.data.search.frase+'&stype=regex');
            }
...
app.controller('AdvancedSearchController',['$scope', '$http', '$log', '$q', '$location','AppService', '$window', '$filter', '$timeout', 'orderByFilter','$route','$routeParams', function($scope, $http, $log, $q, $location,AppService, $window, $filter, $timeout, orderBy,$route,$routeParams) {
    ...
    $scope.$watch('data.all', function(newV, oldV){
        if(Object.keys(newV).length>1){
            $scope.morebutton = true;
            $scope.pags = Object.keys(newV).length-1;
        }
    });

    $scope.onSubmit = function(){

        $http.post(AppService.url+'/ajax/advanced/search/all', $scope.search)
            .then(
                function(response){
                    console.log(response.data);
                    $scope.data.all = response.data;
                },
                function(reason){
        ...
        $http.post(AppService.url+'/ajax/advanced/search/chunk', $scope.search)
            .then(
                function(response){
                    console.log(response.data);
                    $scope.data.results = response.data;
                },
                function(reason){
    ...
    $scope.addNext = function(){
        ...
        if($scope.current<$scope.pags) {
            $scope.current++;

            $scope.data.results.push(
                $scope.data.all[$scope.current]
            )
...
app.config(function($routeProvider, $locationProvider) {
    ...
    $routeProvider.
    when('/', {
        templateUrl: '/templates/front/search/normal.html',
        controller: 'SearchNoAuthController'
    }).when('/zawansowane', {
        templateUrl: '/templates/front/search/advanced.html',
