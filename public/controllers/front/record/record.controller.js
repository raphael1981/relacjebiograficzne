...
var app = angular.module('app',['ngSanitize','ngRoute','ngDialog','ngPhotoSwipe','perfect_scrollbar','ngCookies'], function($interpolateProvider) {
...
app.controller('RecordController',['$scope',
	...
	function($scope,$sce,
		...
		$scope.init = function(data,fragments){
            ...
            //console.log('frase',$scope.frase);
            //$scope.getSearchResult($scope.frase,arguments[2])
			/*
			 $timeout(function(){
			...
			$scope.currentMediaPath = data.type=='audio' ?
					$sce.trustAsResourceUrl('http://fono.dsh.waw.pl/'+data.source) :
					$sce.trustAsResourceUrl('http://video.dsh.waw.pl/'+data.source);
			mySharedService.prepForBroadcast($scope);
		...
		$scope.getSearchResult = function(time, frase){
			...
			$timeout(function(){
				...
				//$scope.searchByPhrase(frase);
				go2Time($scope.player[0],1)
			},2000)
			console.log('newtime',$filter('secondsToTime')(time));
		...
		function go2Time(elem,mod){
			console.log('jestem',elem.currentTime)
		...
		$scope.$watch('search.phrase',function(newVal,oldVal){
			if(newVal != oldVal){
				end = false;
			}
		})
		...
		$scope.searchByPhraseES = function(id) {
			if(end){
				$scope.alertMess('Koniec wyszukiwania')
			}
			if($scope.search.phrase !== '') {
							...
							angular.forEach($scope.sections,function(item,key){
								...
								angular.forEach($scope.searchresult, function(it,k){
									if(item.getElementsByTagName('time')[0].innerHTML == $filter('secondsToTime')(it.start)){
										...
										currTime = it.start;
										essearchCounter = key++;
										if(k >= $scope.searchresult.length-1){
											end = true;
										}
										console.log($scope.searchresult.length + ' x ' + k)
										block = true;
										return true;
									}
								})
		...
		$scope.$watch('search.phrase', function(newVal,oldVal){
			if(newVal !== oldVal){
				angular.forEach($scope.exceptions,function(item){
					console.log(item)
					if(item.toLowerCase() == newVal){
