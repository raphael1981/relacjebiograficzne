...
ngPhotoSwipe.directive('photoGallery', [ function () {
	var template =
			...
			'<a href="{{img.img_link_generate}}" itemprop="contentUrl" data-size="{{img.size}}">' +
			'<div class="curtain onegall"></div>'+
			'<img ng-src="{{img.img_link_generate}}/350" itemprop="thumbnail" alt="{{img.desc}}" class="img-responsive" />' +
			'</a>' +
			'<div class="tlo-gif"><img src="/images/loading.svg" alt="obrazek ładownia przed pełną odsłoną zdjęcia"></div>'+
			'</div>' +
			'</div>' +

			'<figcaption itemprop="caption description">{{img.desc}}</figcaption>' +
			'</figure>';
