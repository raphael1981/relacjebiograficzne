							     ...
							     if(isPossible){
									   ...
									   colImages = document.getElementById('galeria').
										...
										for(var i=0; i<colImages.length; i++){
                                            ...
                                            var imgdomnames = [];
                                            angular.forEach(domImages,function(item){
                                                imgdomnames.push(item.name);
                                             });
                                            ...
                                            var imgdiff = imgnames.slice(imgdomnames.length,imgnames.length)
                                            var newimgnames = imgdomnames.concat(imgdiff);
											if(scope.currSubject != null) {
                                                $http({
                                                    url: 'galleries/data/subject',
                                                    method: 'PUT',
                                                    data: {id: scope.currSubject.id, photos: newimgnames}
                                                }).then(function successCallback(response) {
                                                    console.log(response.data);
                                                }, function errorCallback(response) {
                                                })

                                                console.log('subid ', scope.currSubject.id)
                                            }

                                            console.log('newimgnames ',newimgnames.length);
                                            console.log('diff ',imgdiff.length);
                                            console.log('imgnames ',imgnames.length);
                                            /*
                                            console.log('imgdomnames', imgdomnames.length)
                                            console.log('imgnames', imgnames.length)
                                            console.log('imgdiff', imgnames.slice(imgdomnames.length,imgnames.length))*/
